...
{
  # ── Secrets ────────────────────────────────────────────────────────────────
  # On mantle these live in secrets/mantle.yaml (same values as harbor's), not
  # the host default sops file — override per-secret.
  sops.secrets.nanomdm_api.sopsFile = ../../secrets/mantle.yaml;
  ...
  virtualisation.oci-containers.containers.scep = {
    ...
    ports = [ "10.100.0.3:${toString scepHostPort}:8080" ];
  ...
  virtualisation.oci-containers.containers.nanomdm = {
    ...
    ports = [ "10.100.0.3:${toString nanomdmHostPort}:9000" ];
    volumes = [
  ...
  # On mantle the containers bind to the WireGuard IP 10.100.0.3, so wg0 must be
  ...
  virtualisation.oci-containers.containers.mdmenroll = {
    ...
    ports = [ "10.100.0.3:9991:8080" ]; # 9990 is nanomdm; 9991 is free
    environmentFiles = [ config.sops.templates."scep.env".path ]; # SCEP_CHALLENGE
  };

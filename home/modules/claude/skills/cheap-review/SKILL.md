...
The first line printed is the run dir; the merged list also lands there as `merged.md`, with per-reviewer files beside it. The output starts with `N of M reviewers succeeded` — if some failed, say so when reporting, because a short list may just mean fewer lenses ran. Code goes to third-party providers (OpenRouter); Daniel has approved that for his own and his company's repos.

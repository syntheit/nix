    ...
    (super.gnome-shell.override {
      ...
      (old: {
        ...
        patches = (old.patches or [ ]) ++ [
          ...
          ./patches/powerkey-wake-race-fix.patch # synchronous intended-screen-state (this._intendedScreenOn) + serialized async turn on/off (this._screenTransition, latest-intent-wins reconcile); DECISIONS read intent not the laggy async PowerSaveMode → kills the rapid-press wake/blank race that left the screen stuck off. MUST stay after the three power-key patches above (same file)
